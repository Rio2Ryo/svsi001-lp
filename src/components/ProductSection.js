...
export default function ProductSection() {
  ...
  const fmtPrice = (value, curLang) => {
    if (curLang === "ja") {
      if (typeof value === "string") return value; // 事前整形済みならそのまま
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setErr("");
        const url = `/products.${lang || "ja"}.json`; // 例: /products.ja.json /products.en.json
        const res = await fetch(url, { cache: "no-store" });
        if (!res.ok) {
          // フォールバック: 日本語版→汎用
          const fb = await fetch("/products.ja.json").catch(() => null);
          if (!fb || !fb.ok) throw new Error(`HTTP ${res.status}`);
  ...
  const Card = ({ p, priority = false }) => {
    ...
    return (
      <div className="product-card">
        ...
        {showOriginal && (
          <p className="product-original">{fmtPrice(p.originalprice, lang)}</p>
        )}
