  ...
  const Card = ({ p, priority = false }) => {
    ...
    return (
      <article className="card">
        <div className="thumb">
          {/* intrinsic（width/height）→ absolute にならない */}
          <Image
            src={FIXED_IMAGE_SRC}
            alt={p?.name || "product"}
            width={1000}             // 大きめにしておきCSSで収める
            height={700}
  ...
  return (
    <>
      ...
      <style jsx>{`
        ...
        /* 画像枠：固定高さで全カード同じラインに。ズレない。 */
        .thumb{
          position:relative;          /* absoluteは使わない */
          width:100%;
          height:300px;               /* ← ここで固定（位置ズレ防止） */
          margin:0 auto 12px;
          background:#fff;
          ...
          overflow:hidden;
          padding:0;                  /* 余白でズレないように0 */
        }
        /* 画像は内フィット＋下寄せ、最大で枠に収める */
        .thumbImg{
          max-height:100%;
          width:auto;
          object-fit:contain;
